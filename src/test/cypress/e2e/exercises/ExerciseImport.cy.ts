...
describe('Import exercises', () => {
    ...
    it('Imports programming exercise', () => {
        ...
        programmingExerciseCreation.import().then((request: Interception) => {
            ...
            courseOverview.startExercise(exercise.id!);
            courseOverview.openRunningExercise(exercise.id!);
            programmingExerciseEditor.makeSubmissionAndVerifyResults(exercise.id!, exercise.packageName!, javaPartiallySuccessful, () => {
                programmingExerciseEditor.getResultScore().contains(javaPartiallySuccessful.expectedResult).and('be.visible');
            });
        });
    });
